...
function fetch(element, urlObj, cookieJar, referrer, pool, agentOptions, userAgent, callback, encoding) {
  let req = null;
  if (urlObj.protocol === "data:") {
    process.nextTick(() => {
      try {
        const buffer = parseDataUrl(urlObj.href).buffer;
        callback(null, encoding === null ? buffer : buffer.toString(encoding));
      } catch (err) {
          callback(err, null);
      }
    });
  ...
  if (typeof customLoader === "function") {
    req = customLoader({
      ...
      defaultFetch(fetchCallback) {
        return fetch(element, urlObj, cookieJar, baseUrl, pool, agentOptions, userAgent, fetchCallback, encoding);
      }
    },
    wrappedEnqueued);
  } else {
    req = fetch(element, urlObj, cookieJar, baseUrl, pool, agentOptions, userAgent, wrappedEnqueued, encoding);
  }
  if (req && requestManager) {
